  ...
  <section class="orders-page">
    ...
    <article v-if="lastOrder" class="order-card order-card--active">
      ...
      <div class="order-items">
        <span v-for="item in lastOrder.items" :key="item.cartKey" class="order-chip">
          {{ item.name }} · {{ item.selectedSize }}<template v-if="item.selectedToppings.length"> · {{ item.selectedToppings.join('、') }}</template> x {{ item.quantity }}
        </span>
      </div>
      <div class="order-card__bottom">
        <span>取餐码 {{ lastOrder.pickupCode }}</span>
        <strong>{{ itemPriceText }}</strong>
      </div>
    </article>
    ...
    <article class="order-card">
      ...
      <div class="order-items">
        <span class="order-chip">生椰拿铁 · 大杯 · 燕麦奶 x 1</span>
        <span class="order-chip">伯牙绝弦 · 中杯 · 珍珠 x 1</span>
      </div>
      <div class="order-card__bottom">
        <span>门店自取</span>
        <strong>{{ itemPriceText }}</strong>
    ...
    <article class="order-card">
      ...
      <div class="order-items">
        <span class="order-chip">多肉葡萄 · 超大杯 · 椰果 x 2</span>
      </div>
      <div class="order-card__bottom">
        <span>品牌专区</span>
        <strong>{{ itemPriceText }}</strong>
...
const itemPriceText = siteConfig.labels.priceText
</script>
